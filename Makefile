CFLAGS=-g -Werror -Wall `pkg-config allegro-5 allegro_image-5 allegro_primitives-5 allegro_audio-5 allegro_acodec-5 allegro_ttf-5  --cflags`
LFLAGS=`pkg-config allegro-5 allegro_image-5 allegro_font-5 allegro_primitives-5 allegro_audio-5 allegro_acodec-5 allegro_ttf-5 --libs` -lm
CC=gcc
SRC=src
OBJ=obj
OUT=tetris
...
$(OBJ)/input.o: $(SRC)/input.c $(SRC)/input.h $(SRC)/tetris.h
	$(CC) -c -o $@ $< $(CFLAGS)
